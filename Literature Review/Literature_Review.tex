...
Since the start of the project, the idea for the genre of the game was a 2D maze adventure. Researchers have argued which genres are best suited for educational purposes \citep{moreno2008}, however most of the discussion on educational game design does not revolve around the game genre, but instead on which game elements, pedagogical concepts and player feelings aroused by the game are most important. These aspects were discussed in the literature review and influenced the requirements specification, which influenced the design of the game.
...
A web-based design was adopted to develop the educational game. A web game has several advantages over other forms of video games. Namely, these are:
...
\begin{itemize}
	...
	\item Accessible on most web browsers, without having to implement different versions of the game for each browser. 
	\item Widely accessible to multiple users through a number of different web-based devices. Do not have to create different versions of the game or port the game onto each different device. 
	\item Ease of accessibility. Requires no download, installation or update to plug-ins apart from having a web browser.
...
The following subsections will discuss the elements of the game that either the user directly interacts with, or are influenced by user action.

%The selection of the airfoils will be discussed in the following subsections. The MH45 was used as the primary airfoil and the S822 airfoil, which was used for the model with aerodynamic twist at the tip.

\subsection{Character Functions}



%The idea of character functions
%How these tie into the use of if statements and for loops
