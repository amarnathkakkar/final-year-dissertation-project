...
The following subsections provide an overview of the components of the system that were significant in the creation of this game.

\subsection{Map-Character Interaction}

\subsubsection{Map}

The map is used to refer to the part of the game that shows the current level layout, this includes the walls represented by black lines, the enemies represented by bats, and the exit tile represented by a red square. The implementation of the map is comprised of two elements. One is what the user sees (see Figure \ref{fig:levelsix}), and the second is what the character interacts with. The map that the user sees is created using graphical lines, graphical rectangles and enemy sprite animations. The second element of the map, which the character interacts with, is discussed below.
...
\begin{wrapfigure}{r}{0cm}
    \includegraphics[width=0.35\textwidth,]{charactercontact}
       \captionsetup{justification=centering}
\caption{Programmer's Dungeon character contact points}
\label{fig:charactercontact}
\end{wrapfigure}

The character and enemies are given four points of 'contact'. Figure \ref{fig:charactercontact} shows the contact points, represented by the red squares, for the character. Enemies are also set-up with a similar set of contact points. The character is given X and Y co-ordinates that represent its centre. This was needed to help the character move across the map which will be discussed later on. To derive the contact points, a fixed pixel length is added/subtracted from its centre to get the 4 points. The origin, or the 0,0 co-ordinate is the top left corner of the map. 

\subsubsection{Interaction}

The character-enemy interaction is simple, since both have contact points. The code checks if any of the enemies contact points are within the characters contact point, and if so, they must be touching.

However, the wall and exit tile interaction 
